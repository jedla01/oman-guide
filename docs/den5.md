---
layout: default
title: Den 5
nav_group: days
permalink: /5den/
nav_order: 6
---
# Den 5 — Nizwa, Jabreen / Bahla a Al Hamra

**Datum:** středa 17. března 2027  
**[Plánovaný nocleh]({{ '/ubytovani/' | relative_url }}):** Jebel Shams

Pevnost v Nizwě, Jabreen / Bahla a Al Hamra; poté přesun na Jebel Shams.

## Program

| Část dne | Návrh |
| --- | --- |
| Ráno | Nizwa Fort; případně souq, pokud nevyjde předchozí den |
| Odpoledne | Jabreen / Bahla a Al Hamra |
| Večer | Příjezd na Jebel Shams |

## K doplnění a ověření

- [ ] Ověřit otevírací dobu pevnosti a dalších vybraných památek.
- [ ] Rozhodnout, zda časově vyjde Jabreen i Bahla, nebo vybrat jednu zastávku.
- [ ] Doplnit přesuny a ubytování na Jebel Shams.

## Rezervace a poznámky

[Trasa a mapa]({{ '/trasa/' | relative_url }}).

[Březnové teploty v horách a oblečení s sebou]({{ '/oman/' | relative_url }}#breznove-teploty).

Pracovní návrh — konkrétní časy, rezervace, náklady a odkazy na mapu zatím doplníme.

[← Den 4]({{ '/4den/' | relative_url }}) · [Přehled cesty]({{ '/' | relative_url }}) · [Den 6 →]({{ '/6den/' | relative_url }})
