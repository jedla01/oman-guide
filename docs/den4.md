---
layout: default
title: Den 4
nav_group: days
permalink: /4den/
nav_order: 5
---
# Den 4 — Východ slunce v poušti a Nizwa

**Datum:** úterý 16. března 2027  
**[Plánovaný nocleh]({{ '/ubytovani/' | relative_url }}):** Nizwa

Východ slunce v Sharqiya Sands, poté přesun do Nizwy s možnou zastávkou v Ibra nebo Birkat Al Mouz.

## Program

| Část dne | Návrh |
| --- | --- |
| Ráno | Východ slunce v poušti |
| Odpoledne | Přesun směrem na Nizwu; Ibra / Birkat Al Mouz podle zvolené trasy |
| Večer | Nizwa a případně souq |

## K doplnění a ověření

- [ ] Vybrat zastávku Ibra nebo Birkat Al Mouz podle času a trasy.
- [ ] Ověřit dobu návštěvy souqu; návštěvu plánujeme před pátkem.
- [ ] Vybrat ubytování v Nizwě.

## Rezervace a poznámky

[Trasa a mapa]({{ '/trasa/' | relative_url }}).

Pracovní návrh — konkrétní časy, rezervace, náklady a odkazy na mapu zatím doplníme.

[← Den 3]({{ '/3den/' | relative_url }}) · [Přehled cesty]({{ '/' | relative_url }}) · [Den 5 →]({{ '/5den/' | relative_url }})
